...
const releaseTypeList: ReleaseType[] = ['major', 'premajor', 'minor', 'preminor', 'patch', 'prepatch'];
const mapedPkgInfoList = new Map(); // 按包名建立信息索引，方便快速访问
const sortedPkgInfoList: PkgInfo[] = []; // 排序根据依赖层级排序的，包列表，最终的处理队列

// read & sort pkg info
function settlePkgInfo() {
  const dirs = readdirSync(pkgsDir);
  const queuePkgInfoList: PkgInfo[] = [];
  dirs.forEach((p: string) => {
    const pkgDir = path.resolve(pkgsDir, p);
    const pkgConfPath = path.resolve(pkgDir, 'package.json');
    const pkg = JSON.parse(fs.readFileSync(pkgConfPath, 'utf-8'));
    const pkgInfo: PkgInfo = {
      name: pkg.name,
      dir: pkgDir,
      deps: Object
        .entries(pkg.dependencies)
        .filter(([_, ver]) => ((ver as string).startsWith('workspace:')))
        .map(([n]) => n),
      sort: 0,
      included: 0,
      conf: pkg,
...
async function progressItem() {
  if (pIndex === sortedPkgInfoList.length)
    return;
  ...
  if (curPkg.deps.length) {
    ...
    maxReleaseTypeIndex = curPkg.deps.reduce((pre, cur) => {
      ...
      return Math.min(pre, depReleaseType
    ...
    if (maxReleaseType.includes('pre')) {
      curTypeList = curTypeList.filter(i => i.startsWith('pre'));
    }
  }
  ...
  let releaseType: ReleaseType = 'major';
